...
const newNumbers = numbers.map((number) => number ** 3 + 55);
...
const onlyMoreThan4DigitsFruits = fruits.filter((fruit) => returnfruit.length > 4);

// const onlyStrings = newArr.filter(function (item, index) {
//   return typeof item === "string" || item > 5;
// });

const onlyStrings = newArr.filter(item => typeof item === "string" || item > 5);
